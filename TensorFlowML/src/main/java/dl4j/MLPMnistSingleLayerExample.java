...
public class MLPMnistSingleLayerExample {
    ...
    public static void main(String[] args) throws Exception {
        //number of rows and columns in the input pictures (in this case, MNIST pictures are 28x28 pixels)
        final int numRows = 28;
        final int numColumns = 28;
        int outputNum = 10; // number of output classes (i.e. results), in this case it's 10 digits
        int batchSize = 128; // batch size for each epoch (chosen based on experience)
        int rngSeed = 123; // random number seed for reproducibility (i.e. for each run, weights are initialized the same way)(chosen based on experience)
        int numEpochs = 15; // number of epochs (runs through data) to perform

        // Get the DataSetIterators:
        // Fetching data (e.g. MNIST in this case)
        // Use two partitions: training and testing (at least in supervised training)
